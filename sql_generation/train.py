...
def parse_option():
    ...
    parser.add_argument('--ckpt_num', type=int, default=20,
                        help="The number of ckpts during training. (uniform sampling)")
    ...
    # args for graph of thought
    parser.add_argument('--use_graph', action='store_true', help="Whether to use GoT to fine-tune the model")
    parser.add_argument('--max_node_num', type=int, help="The number of nodes in the graph")
    parser.add_argument('--max_node_len', type=int, help="The length of the tokens in the node")
...
def train(opt):
    ...
    model.model.init_extracted_modules()
    ...
    optimizer = AdamW(trainable_params, lr=opt.lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.1)
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.print("GPU 0, step {}, loss {}".format(global_completed_steps,
                                                                   accumulation_loss / accelerator.gradient_accumulation_steps))
                accelerator.print("GPU 0, step {}, lr state dict:".format(global_completed_steps),
                ...
                if global_completed_steps % checkpointing_steps == 0:
                    accelerator.print(
                        "after {} global training steps, save a checkpoint".format(global_completed_steps))
